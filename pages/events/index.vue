        ...
        <v-layout row wrap>
          <v-flex xs12 md8 offset-md2>
            ...
            <v-card class="three-cont" v-if="!!nextEvent && nextEvent.image">
              <v-img :src="nextEvent.image">
                <div class="main-event-title-holder">
                  <span class="main-event-title">{{nextEvent.title}}</span>
                </div>
              </v-img>
              <v-content>
                <v-divider></v-divider>
                ...
                <p
                  v-else
                >{{new Date(nextEvent.start_date).toDateString()}} - {{new Date(nextEvent.end_date).toDateString()}}</p>

                <p
                  v-if="nextEvent.free"
                >Free Event - book by {{new Date(nextEvent.bookings_close).toDateString()}}</p>
                <p
                  v-else-if="nextEvent.cost === 0"
                >By donation - Bookings close {{new Date(nextEvent.bookings_close).toDateString()}}</p>
                <p
                  v-else
                >Event Cost ${{nextEvent.cost.toFixed(2)}} - Bookings close {{new Date(nextEvent.bookings_close).toDateString()}}</p>
                <v-divider></v-divider>
                <br />
                <h4>{{nextEvent.subtitle}}</h4>
                <br />
              ...
              <br />
              <v-btn
                v-if="bookingsOpen(nextEvent.bookings_close) && nextEvent.bookings_open"
                color="rgb(219, 155, 59)"
                outline
                nuxt
                :to="'/events/register/'+nextEvent.id"
              >
                <v-icon>how_to_reg</v-icon>Register now
              </v-btn>
              <v-btn v-else color="rgb(219, 155, 59)" outline disabled>
                <v-icon>how_to_reg</v-icon>Registrations Closed
              </v-btn>
            </v-card>
          </v-flex>
        </v-layout>
          ...
          <v-flex xs12 sm10 offset-sm1 xl8 offset-xl2>
            <v-layout row wrap>
              <v-flex v-for="event in otherEvents" :key="event.title" xs12 md6 lg4>
                <v-card
                  style="margin:18px; outline: 1px solid rgb(157,157,157); outline-offset: 12px; min-height:760px;"
                >
      ...
      meta: [
        {
          hid: "description",
          name: "description",
          content: "The events page of Ordinary Goddesses"
        }
      ]
    };
  },
